...
export function Overview() {
  ...
  const columns = useMemo<TableColumnDef<Emission>[]>(
    () => [
      ...
      ...baseColumns,
      {
        id: 'actions',
        header: '',
        meta: { width: 100, type: 'button', align: 'center', exportable: false },
        enableSorting: false,
        cell: (ctx) => (
          <button
            type='button'
  ...
  return (
    <DemoPage
      title='한눈에 보기'
      summary='선택, 정렬, 비활성 행, 행 클릭, 액션 셀을 한 테이블에 모은 예제입니다. 기능별로 나눈 설명은 왼쪽 메뉴에 있습니다.'
      customization={[
        <>
          컬럼은 TanStack Table의 <Code>ColumnDef</Code> 그대로입니다. 폭·정렬·셀 타입은 <Code>meta</Code>로 지정합니다.
        </>,
        <>
          색상·간격·테두리는 전부 <Code>--tbl-*</Code> CSS 변수입니다. 오른쪽 테마 편집기에서 바로 바꿔볼 수 있습니다.
        </>,
        <>
          체크박스와 정렬 아이콘은 <Code>components</Code>로 교체할 수 있습니다.
        </>,
      ]}
      api={[
        ['data / columns', 'TData[] / ColumnDef[]', 'TanStack Table과 동일합니다.'],
        ['getRowId', '(row, i) => string', '행의 고유 키. 선택·확장·드래그가 모두 이 값을 씁니다.'],
        ['labels', 'TableLabels', '로딩·빈 상태·체크박스 등 화면에 읽히는 모든 문자열.'],
        ['getRowDisabled', '(row) => boolean', '행을 비활성 처리합니다. 클릭도 선택도 되지 않습니다.'],
        ['onRowClick', '(row) => void', '행 클릭. 셀 안의 버튼·체크박스 클릭은 전파되지 않습니다.'],
      ]}
    >
      <div className='space-y-3'>
        ...
        />
        <Note>
          선택: <Code>[{selected.join(', ') || '—'}]</Code> · 정렬:{' '}
          <Code>{sorting[0] ? `${sorting[0].id} ${sorting[0].desc ? 'desc' : 'asc'}` : '—'}</Code> · 마지막 클릭:{' '}
          <Code>{clicked ?? '—'}</Code>
        </Note>
        <Note>
          5행과 8행은 <Code>getRowDisabled</Code>로 비활성입니다. 정렬 헤더는 실제 버튼이라 Tab으로 이동해 Enter로
          정렬됩니다.
        </Note>
      </div>
    </DemoPage>
